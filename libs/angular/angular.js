...
function $HttpProvider() {
  ...
  this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector',
     ...
     *       // or client returns response with an error status.
     *     });
     * ```
     *
     ...
     * When the cache is enabled, `$http` stores the response from the client in the specified
     ...
     * cache, but the cache is not populated yet, only one request to the client will be made and
     ...
     * able to intercept requests before they are handed to the client and
     ...
     *
     * Both client and the client must cooperate in order to eliminate these threats. Angular comes
     * pre-configured with strategies that address these issues, but for this to work backend client
     ...
     * counter this your client can prefix all JSON requests with following string `")]}',\n"`.
     * Angular will automatically strip the prefix before processing it as JSON.
     *
     * For example if your client needs to return:
     * ```js
     * ['one','two']
     * ```
     *
     * which is vulnerable to attack, your client can return:
     * ```js
     * )]}',
     * ['one','two']
     * ```
     ...
     * JavaScript that runs on your domain could read the cookie, your client can be assured that
     * the XHR came from JavaScript running on your domain. The header will not be set for
     * cross-domain requests.
     *
     * To take advantage of this, your client needs to set a token in a JavaScript readable session
     * cookie called `XSRF-TOKEN` on the first HTTP GET request. On subsequent XHR requests the
     * client can verify that the cookie matches `X-XSRF-TOKEN` HTTP header, and therefore be sure
     * that only JavaScript running on your domain could have sent the request. The token must be
     * unique for each user and must be verifiable by the client (to prevent the JavaScript from
     ...
     *      HTTP headers to send to the client. If the return value of a function is null, the
 ...
 *
 * For the case of HTML, you might use a library, either on the client side, or on the client side,
 * to sanitize unsafe HTML before binding to the value and rendering it in the document.
 *
 * How would you ensure that every place that used these types of bindings was bound to a value that
 * was sanitized by your library (or returned as safe for rendering by your client?)  How can you
 ...
 * for those values that you can easily tell are safe - because they were received from your client,
 ...
 * page reload that sends the data to the client. Instead some javascript logic should be triggered
 * to handle the form submission in an application-specific way.
 *
 * For this reason, Angular prevents the default action (form submission to the client) unless the
 ...
 * client and reloading the current page), but only if the form does not contain `action`,
